...
@section('content')
    ...
    <div class="md-card uk-margin-medium-bottom">
        <div class="md-card-content">
            ...
            <form action="{{ route('teacherattendance_update') }}" method="post">
                {{ csrf_field() }}
                ...
                <table id="t_dt_search" class="uk-table" cellspacing="0" width="100%">
                    ...
                    @foreach($users as $user)
                        ...
                        @if($user->date == date("Y-m-d"))
                            <tr>
                                <td style="display: none"><input type="text" name="staff_id[]" value="{{ $user->atten_id }}"></td>
                                <td>{{ $user->first_name }}</td>
                                <td>{{ $user->name }}</td>

                                 @if($user->status ==1)
                                <td><input type="checkbox"  checked name="status[{{ $user->atten_id }}]"> </td>
                                 @else
                                    <td><input type="checkbox"   name="status[{{ $user->atten_id }}]"> </td>
                                @endif
    ...
    <div class="md-fab-wrapper">
        <a class="md-fab md-fab-accent" href="{{ route('teacherattendance_create') }}" id="recordAdd">
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $(".new_date").on('change',function() {
                ...
                var data = '';
                $.ajax({
                    url: "{{ route('teacherattendance_getAllStaff') }}",
                    type: "GET",
                    data: {att_date: attendancedate},
